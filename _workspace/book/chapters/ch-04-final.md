...
---
<!-- 소스 포스트: 리더십팀은 UN이 아니다, 개발 조직에서의 리더 (부제: 다양한 문제해결 스타일) -->
<!-- spec 커버리지:
- [x] 핵심 논지: 리더십팀에서 첫 번째 팀은 리더십팀 자체, 하나의 팀 하나의 점수
- [x] UN 총회 메타포: 대사 vs 리더의 본질적 차이
- [x] 리더십 미팅 오프닝 장면
- [x] 축구 팀 메타포: 하나의 팀, 하나의 점수 (패스 성공률 94% but 0-3 패배)
- [x] 왜 부서 이익을 내려놓기 어려운가 (정체성, 팀원의 기대, 배신자 두려움)
- [x] 조직의 리더로 서는 3가지 연습 (주어 바꾸기, 다른 부서 문제에 나서기, 결정 진심으로 지지하기)
- [x] MOI 모델 (Motivation, Organization, Idea)
- [x] 문제 해결형 리더의 3가지 역할: 문제 이해, 아이디어 흐름 관리, 품질 유지
- [x] "리더는 조직에서 임명된 리더만이 아니라, 변화를 만들어내는 모든 사람"
- [x] 전환: Ch.03(채용)에서의 연결 → Ch.05(명료함)로의 연결
- [x] MOI 23가지 행동 패턴 중 5~6개 선별 반영
- [x] 제럴드 와인버그 출처를 "실제 적용 경험" 톤으로 언급
-->
