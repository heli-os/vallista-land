...
비유는 멋스러움보다 즉각적 이해가 우선이다. 초고를 마치면 모든 비유의 매개체에 대해 "처음 보는 독자가 즉시 이미지를 떠올릴 수 있는가"를 자문하고 NO면 일상어로 교체한다.

- **회피**: 수학·프로그래밍·물리 전문 용어 매개 (`0에 수렴`, `환경의 함수다`, `객체`, `런타임`, `엔트로피`)
- **회피**: 매개체가 대상과 결이 안 맞는 비유 (디지털 도구에 `잉크`, 비물리 개념에 `무너진다` 같은 과장 동사)
- **회피**: 의인화 비유 중 본문에서 동작 이유가 풀리지 않는 것 (`텍스트가 사라지지 않으려 애쓴다`)
- **회피**: 같은 단어가 사실 묘사와 은유로 동시에 쓰여 의미 충돌하는 경우 (앞에서 "침묵의 30분"으로 사실 묘사한 단어를 뒤에서 "메모의 침묵"으로 은유 재사용)
- **회피**: 매개체 자체가 본문에서 풀리지 않는 추상 단어 비유 (`독자의 도서관`, `발표자의 갑옷`)
- **권장**: 직관 단어 + 풀어쓴 동작으로 교체 (`발표자의 보호막`, `독자가 따져 읽는 시선`)

핵심 메타포(글 전체 관통 1~2개)는 보존, 그 외 부수 비유는 모호하면 평어로 풀어쓴다. 자세한 사유는 메모리 `feedback_essay_metaphor.md` 참조.
