...
최근 AI 에이전트의 인지 구조를 분석한 논문을 읽었다. 인간의 기억 체계를 빌려와 에이전트를 설계하는 프레임워크였는데, 기억을 네 가지로 나눈다. 지금 이 순간 쓰이는 정보를 담는 작업 기억, 과거 경험을 저장하는 일화 기억, 세계에 대한 사실을 보관하는 의미 기억, 그리고 "어떻게 하는지"를 담는 절차 기억.
...
그래서 멘토십이, 페어 프로그래밍이, 도제식 학습이 AI 시대에도 사라지지 않는 것이 아닐까. 명시적 절차 기억은 문서로 전달할 수 있다. 하지만 암묵적 절차 기억은 옆에서 보고, 따라 하고, 실패하고, 다시 시도하는 과정을 통해서만 전달된다. 자전거를 타는 법을 배우듯이.
