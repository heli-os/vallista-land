...
네 편의 논문이 그린 궤적은 이렇다. 생각할 수 있다(CoT), 생각하면서 행동할 수 있다(ReAct), 행동을 스스로 배울 수 있다(Toolformer). 하지만 이 모든 것은 한 명의 에이전트가 홀로 수행한 것이다. 이번 논문은 지난 글의 마지막 문장에 담긴 질문, "이 모든 것을 하나의 시스템 안에서 어떻게 조율하는가?"에 답한다.
