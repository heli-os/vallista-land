...
메모 회의가 어색한 이유도 여기에 있다. 발표자의 갑옷이 사라지고 독자의 도서관이 들어선다. 도서관은 친절하지 않다. 친절은 발표자의 일이었지, 독자의 일이 아니다.
...
프로토타입이 책상 위에 올라오면 회의의 문법이 다시 한 번 바뀐다. 자료가 아니라 객체다. 텍스트는 사라지지 않으려 애쓰는데, 객체는 그저 만져지길 기다린다. 발표자의 갑옷도, 독자의 도서관도 거기서는 작동하지 않는다. 사용자가 한 번 클릭하면, 다섯 페이지의 메모와 스무 장의 슬라이드가 동시에 무력해지는 순간이 있다.
