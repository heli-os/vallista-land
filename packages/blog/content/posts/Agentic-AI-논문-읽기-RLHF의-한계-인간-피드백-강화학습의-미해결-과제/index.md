...
논문 자체도 인정하듯, RLHF의 대안으로 제시된 접근법들(DPO, RLAIF, 과정 기반 보상)은 논문 시점에서 초기 단계였다. 이 대안들이 실제로 어떤 문제를 해결하고 어떤 새로운 문제를 낳았는지는, 논문의 범위를 넘어선다. 하지만 문제 목록이 있다는 것 자체가, 해결책을 평가하는 체크리스트가 된다. "이 새로운 기법은 표의 어떤 행을 해결하는가?"라는 질문을 던질 수 있게 된 것이 서베이의 기여다.
