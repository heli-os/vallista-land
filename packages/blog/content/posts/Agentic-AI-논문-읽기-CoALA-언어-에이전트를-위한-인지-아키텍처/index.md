...
- 지금 읽고 있는 문장의 내용(작업 기억)
- 어제 저녁에 먹은 것(일화 기억)
- "서울은 한국의 수도"라는 사실(의미 기억)
- 자전거 타는 법(절차 기억)

CoALA는 이 인지심리학의 분류를 에이전트 설계에 그대로 가져온다.
...
논문은 "더 강력한 LLM이 등장하면 외부 기억이나 도구가 불필요해질까?"라고도 물었다. 실제로 컨텍스트 윈도우는 백만 토큰을 넘겼고, LLM의 추론 능력도 비약적으로 발전했다. 하지만 CoALA의 예측대로, 체계적인 기억과 행동의 구조는 사라지지 않았다. 오히려 에이전트가 더 복잡한 과제를 다루게 되면서 그 필요성이 더 뚜렷해졌다. 달라진 것은 각 모듈의 구현 방식과 비중이다.
